...
> Nguồn: docs/research/02-editor-annotation.md · Module: `ezsnag_canvas`, `ezsnag_annotations`, `ui/EditorWindow`
...
> Tất cả annotation tool, Move tool, và Effects đều **chỉ áp dụng cho ảnh** (image-only). Video không annotate trong editor này (Snagit chuyển sang Camtasia / Screen Draw); xem SPEC 05.
...
| Region | Vị trí | Vai trò | Lớp clone |
|---|---|---|---|
...
| **Assets panel** | Mở từ Editor | TechSmith Assets (stamps/themes/templates) | Clone: thư viện stamp/theme local; cloud Assets = tạm gác (SPEC 00 §4) |
...
- Clone: `QToolBar` cho phép ẩn/hiện action; lưu thứ tự + visibility vào `QSettings`.
...
- **Editable format:** `.SNAG/.SNAGX` lưu mọi object + state → mở lại edit tiếp. Clone dùng **`.ezsnagx`** (SPEC 09).
...
> *Trong Snagit, Blur được docs xếp gần nhóm raster, nhưng nó còn re-editable như object (chọn lại đổi intensity). Clone: ưu tiên hiện thực **Blur/Pixelate là object vector** (re-edit được), flatten khi export. Xem §02.17.

### Ánh xạ clone C++/Qt — quyết định kiến trúc
...
> **Universal UX flow** (research §4, dùng chung, viết tắt ở mỗi tool): chọn tool trên toolbar → Quick Styles + Tool Properties hiện bên phải → chọn Quick Style hoặc chỉnh property → click (hoặc click-drag) trên canvas tạo object → gõ/điều chỉnh → chuyển sang **Move tool** để chọn lại, di chuyển, restyle, hoặc xóa.

> **Convention clone:** trừ khi ghi rõ là raster, mọi tool tạo một `QGraphicsItem` subclass (đặt tên `Ezsnag<Tool>Item`) backed bởi một `AnnotationObject` trong model. Selection handles (white=resize, green=rotate, yellow/blue=tail…) hiện thực bằng các child item / overlay handle khi item được chọn ở Move tool.
...
- **Clone:** `EzsnagLineItem : QGraphicsItem` (chia sẻ nhiều logic với Arrow — có thể chung base `EzsnagStrokeItem`). Khi raster mode → vẽ thẳng vào `backgroundRaster`.
...
- **Custom stamps:** import **JPG/PNG/GIF**. Mac có thể lưu ảnh Snagit thành custom stamp.
- **Assets:** hàng trăm stamp free từ TechSmith Assets (Win: **Download More**; Mac: browse/import). → Clone: thư viện stamp **local** đóng gói; cloud Assets tạm gác (SPEC 00 §4).
- **Properties:** Shadow · Opacity. Lưu Quick Styles.
- **Ưu tiên:** **MVP P3** (đặt stamp từ thư viện local + import PNG; GIF động + cloud = sau).
- **Clone:** `EzsnagStampItem : QGraphicsPixmapItem` (giữ ảnh gốc để re-scale nét). Animated GIF = tạm dùng frame đầu ở MVP.
...
- **Clone:** `EzsnagPenItem : QGraphicsItem` giữ `QPainterPath` (sample điểm khi drag; smoothing = simplify path / Catmull-Rom). Raster mode → `QPainter` vẽ path thẳng vào `backgroundRaster`. Round-cap pen shape = `Qt::RoundCap`.
...
- **Clone:** **raster op** trên `backgroundRaster`. Scanline/queue flood fill trên `QImage`, so màu theo tolerance (khoảng cách RGB). `QUndoCommand` lưu before/after bounding region. KHÔNG phải `QGraphicsItem`.

### 02.11 Eraser — RASTER
- **Mô tả:** xoá vùng flattened để lộ canvas dưới. Drag qua pixel cần xoá.
- **Ưu tiên:** **sau** (P4+).
- **Clone:** raster op — vẽ trong suốt (clear alpha) lên `backgroundRaster` dọc đường drag với brush tròn. Undo lưu region.
- **Điểm chưa chắc (research §9.1):** kích thước/shape brush của Eraser **không** rõ trong docs gốc → clone tự đề xuất brush size slider + round brush; verify sau.
...
- **Clone:** vùng chọn = `QPainterPath`/mask trên `backgroundRaster`. Cut/copy → `QImage` sub-region vào clipboard (`QClipboard`). Move/delete + Transparent = clear pixel; **Auto-Fill** = inpainting (cân nhắc OpenCV `inpaint`; tạm gác). Snap to Object = edge detection nhẹ.
...
- **Clone:** Move tool = chế độ tương tác của `CanvasView`: bật `QGraphicsItem::ItemIsSelectable | ItemIsMovable`, hiện selection handles (resize/rotate) cho item chọn, route Del/double-click. **Smart Move** thuộc `ezsnag_simplify` (SPEC 04) — không hiện thực trong MVP của spec này.
...
- **Clone:** raster transform trên `backgroundRaster`: cắt 2 nửa, ghép lại (đổi canvasSize). Vì làm đổi geometry nền, mọi object vector phải dịch theo nếu nằm sau dải cắt. `QUndoCommand` lưu ảnh + offset object.
...
- **Ưu tiên:** **sau** (P4+).
- **Clone:** `EzsnagMagnifyItem : QGraphicsItem`. Khi paint: lấy vùng nguồn từ ảnh nền (+ object dưới), scale theo %, vẽ vào inset rect với outline. Cần access composite-below → render-on-demand từ document; cân nhắc cache `QPixmap`.
...
- **Clone:** widget eyedropper trong color picker. Trong canvas: `QImage::pixelColor` tại điểm click. Toàn màn hình: grab screen (`QScreen::grabWindow`) rồi pick — nền tảng-specific.
...
- **Clone:** z-order ↔ `zIndex` trong model + `setZValue`. Commands: BringToFront / SendToBack / BringForward / SendBackward (mỗi cái = `ReorderZCommand` trên undo stack). Group = item cha `QGraphicsItemGroup` chứa child; transform chung.
- **Điểm chưa chắc (research §9.2, §9.6):** menu label + shortcut group/arrange chính xác **không** rõ trong docs → clone đặt label riêng (Bring to Front…), verify sau.
...
- **Clone:** per-object rotate = `setRotation` quanh `transformOriginPoint`. Image flip/rotate = transform `backgroundRaster` (`QImage::mirrored`, `QTransform().rotate(90)`) + dịch/transform mọi object theo. Image-level thuộc `ezsnag_effects` về bản chất (SPEC 03) nhưng menu ở Editor.
- **Điểm chưa chắc (research §9.3):** per-object flip vs image-level flip label chưa rõ — verify.
...
- Tạo theme → Snagit **auto-generate Quick Styles** từ màu theme, xuất hiện ở hầu hết tool.
- **Tạo:** chọn màu (preset/palette/HEX/RGB/eyedropper) → pick font → đặt tên → Create. **Edit/Export:** gear icon → export **.SNAGTHEME** chia sẻ; import; tải theme pro từ Assets.
- **Đây là cơ chế brand colors** của Snagit — định nghĩa 1 lần, lan ra Quick Styles khắp nơi.
- **Ưu tiên:** **sau** (P4+) — color picker cơ bản đủ cho MVP; theme system + auto-gen Quick Styles = mở rộng.
- **Clone:** `Theme` (palette + font + shadow flag) serialize **`.ezsnagtheme`** (đổi tên format); generator tạo `ToolStyle` từ màu theme. Cloud theme download = tạm gác.
...
- **Clone:** `ui/ColorPickerWidget` dùng lại mọi nơi; gradient = sau.
...
- **Clone:** `QUndoStack` trong `CanvasDocument`; mỗi thao tác object = `QUndoCommand`. Edit/View menu nối `createUndoAction/createRedoAction`. **MVP P3** (cốt lõi).
...
- **Clone:** copy object → serialize JSON vào clipboard (mime riêng) + raster vào `QClipboard`; paste tạo object/`QGraphicsPixmapItem` mới offset. **MVP P3.**
- **Điểm chưa chắc (research §7):** phím Duplicate riêng (Ctrl/Cmd+D?) **không** xác nhận trong docs → clone tự gán Ctrl+D, verify.
...
- **Clone:** snapping = khi kéo, so cạnh/tâm bounding rect các item, hút trong ngưỡng px, vẽ guide line tạm. Toggle lưu preference. **MVP P3** (Shift-constrain bắt buộc; smart guide snapping P3+).
- **Điểm chưa chắc (research §6, §9.5):** lệnh **align/distribute** rõ ràng (ngoài Canvas Snapping) **không** xác nhận → clone có thể thêm align/distribute riêng, verify.
...
Giữ nguyên các uncertainty từ research §9 + bổ sung quyết định clone:

1. **Eraser** — kích thước/shape brush không rõ trong docs (§9.1). Clone tự đề xuất brush-size slider + round brush; verify build hiện tại.
2. **Group / Arrange** — menu label + shortcut chính xác chưa rõ (§9.2, §9.6). Clone đặt label riêng (Bring to Front / Send to Back…); verify.
