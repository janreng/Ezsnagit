...
> Nguồn: docs/research/05-library-share-templates.md · Module: `ezsnag_canvas`, `ezsnag_library`

## Mục tiêu & phạm vi

Định nghĩa các định dạng Ezsnagit đọc/ghi, trọng tâm là **`.ezsnagx`** — định dạng native giữ **object vector** để re-edit — và **rule flatten** khi export sang raster/PDF/video. Insight cốt lõi (kế thừa `.snagx`→`.ezsnagx` của Snagit): chỉ `.ezsnagx` đảm bảo edit lại được; mọi export khác là **phẳng** (baked-in pixel).
...
- **Ghi chú clone C++/Qt (.ezsnagx store, serialize):**
  ...
  - **Cross-platform:** JSON + PNG + ZIP đều portable → file tạo trên Win mở được trên Mac (giữ tinh thần `.snagx` cross-platform của Snagit). Version field trong `document.json` để migrate.

### 09.2 Image formats — import & export

- **Mô tả (kế thừa Snagit, lọc theo khả thi Qt):**
  - **Import:** PNG, JPG, BMP, GIF, TIF, ICO (+ `.ezsnagx`). Các định dạng hiếm của Snagit (RAS, EMF, PSD, TGA, WFX, WMF, WPG) → tùy plugin/thư viện, không bắt buộc bản đầu.
...
- **Ghi chú clone C++/Qt:** encode FFmpeg (H.264). Không có định dạng video native editable — video là output phẳng.
...
- **Mô tả:**
  ...
  - (Hành vi Mac của Snagit) khi quit, file **non-`.ezsnagx`** đang mở bị flatten, annotation cũ không sửa được nữa.
...
- **Ghi chú clone C++/Qt (rule flatten khi export PNG/JPG):**
...
- Snagit liệt kê cả **`.snag`** lẫn `.snagx`; clone chỉ làm **một** native (`.ezsnagx`), bỏ legacy.
...
- Hành vi **flatten-on-quit** kiểu Mac có nên port không, hay luôn giữ `.ezsnagx` editable bất kể nền tảng — đề xuất **luôn giữ editable**, không bắt chước flatten-on-quit (an toàn hơn cho user).
